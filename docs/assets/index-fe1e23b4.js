...
const skeletonSprite = "" + new URL("Skeleton enemy.png", import.meta.url).href;
...
const _Sprite = class {
  ...
  getSpriteByIndex(index) {
    const fromCache = _Sprite.cache.get(index, this.filter ?? null, this.config);
    if (!!fromCache) {
      return fromCache;
    }
    ...
    let rowIndex;
    let colIndex;
    if (this.config.rowIndex !== void 0) {
    ...
    _Sprite.cache.set(index, this.filter ?? null, this.config, image);
    return image;
  }
};
let Sprite = _Sprite;
...
class ResourceLoader {
  ...
  static flipImage(image, rect, key = 1) {
...
__publicField(ResourceLoader, "flippedImagesMap", new TrippleMap());
__publicField(ResourceLoader, "loadedImagesCache", /* @__PURE__ */ new Map());
...
class Rotation {
  ...
  isLeft() {
    ...
    const toReturn = Math.abs(value) > Math.PI / 2;
    return toReturn;
  }
  isRight() {
    return !this.isLeft();
...
class Box {
  ...
  static fromMarkupRect(markupRect, { scale = 1 } = {}) {
...
const _FireAttackEntity = class extends FireEntity {
  constructor(x, y, rotation = 0, speed = _FireAttackEntity.DEFAULT_SPEED, width = _FireAttackEntity.DEFAULT_WIDTH, height = _FireAttackEntity.DEFAULT_HEIGHT, filter) {
    ...
    __publicField(this, "MAX_FIRE_TIME_MS", 6e4 * 0.2);
...
class PlayerEntity extends DrawableEntity {
  constructor(x, y) {
    ...
    __publicField(this, "finiteStateMachine");
    __publicField(this, "inputController");
    __publicField(this, "health", new Health(1e4, 1e4));
...
class BaseEnemyGameObject extends GameObject {
  constructor(x, y, enemy, finiteStateMachine, width = 100, height = 100) {
    super(finiteStateMachine);
    __publicField(this, "enemy");
    __publicField(this, "attackCircle");
    __publicField(this, "healthBar");
...
class TeleportEntity extends DrawableEntity {
  constructor(boxIn, boxOut) {
    super();
    __publicField(this, "unsubscribeFromPlayerMoveFn", null);
    __publicField(this, "boxOut");
    this.gameObject = new GameObject();
  ...
  initEntity() {
    super.initEntity();
    this.unsubscribeFromPlayerMoveFn = this.getEntityManager().emitter.subscribe("player_move", (player) => {
...
const STAIRS_TOP_LEFT_WALL_PATH = [
  {
    "x": 471,
    "y": 5
  },
...
const DEFAULT_WIDTH$1 = 1580;
const DEFAULT_HEIGHT$1 = 1720;
class TopLeftStairsWallEntity extends DrawableEntity {
  constructor(x, y, scale) {
    ...
    const box = new Box(
      new Position(x, y),
      DEFAULT_WIDTH$1 * scale,
      DEFAULT_HEIGHT$1 * scale
    );
    this.gameObject = new GameObject();
...
class Stairs extends DrawableEntity {
  constructor(x, y, scale = 2) {
    ...
    __publicField(this, "topStairsTeleportBox");
    __publicField(this, "bottomStairsTeleportBox");
    __publicField(this, "stairsSpawnArea");
    this.x = x;
    this.y = y;
    this.scale = scale;
  ...
  getTopStairsTeleportBox() {
    return this.topStairsTeleportBox;
  }
  getBottomStairsTeleportBox() {
...
class SlimeSpawner extends Spawner {
  spawn() {
    return new SlimeEntity(0, 0);
  }
}
class SpawnArea extends Entity {
  constructor(box, spawner, maxCount, CHECK_INTERVAL, CLEAR_DEATH_ENTITIES_CHECK_INTERVAL) {
    super();
    __publicField(this, "liveEntities", []);
    __publicField(this, "deadEntities", []);
    __publicField(this, "unsubscribeFromEnemyDeathFn", null);
    __publicField(this, "timeSinceLastEntitiesCountCheck");
    __publicField(this, "timeSinceLastClearDeathEntititesCheck", 0);
    this.box = box;
    this.spawner = spawner;
    this.maxCount = maxCount;
    this.CHECK_INTERVAL = CHECK_INTERVAL;
    this.CLEAR_DEATH_ENTITIES_CHECK_INTERVAL = CLEAR_DEATH_ENTITIES_CHECK_INTERVAL;
...
const _StairsLocation = class extends Stairs {
  constructor(x, y) {
    super(x, y, _StairsLocation.SCALE);
    __publicField(this, "slimeSpawnArea", null);
    __publicField(this, "isPlayerEntered", false);
  }
  initEntity() {
    super.initEntity();
  ...
  onPlayerEnter() {
    const followPlayerCamera = this.getEntityManager().getEntityByName("camera");
  ...
  onPlayerExit() {
    const followPlayerCamera = this.getEntityManager().getEntityByName("camera");
    followPlayerCamera.params = void 0;
...
let StairsLocation = _StairsLocation;
__publicField(StairsLocation, "SCALE", 1.5);
const HOME_MAP_TREES_POSITIONS = [
  {
...
const HOME_MAP_NAMED_LOCATIONS = {
  home: [
    { x: 40, y: 40 },
    { x: 41, y: 41 }
  ],
...
class SkeletonEntity extends BaseEnemyEntity {
  constructor(x, y) {
    super(x, y);
    __publicField(this, "health", new Health(250, 250));
...
class HomeMap extends GameMap {
  ...
  initialize() {
    super.initialize();
    this.addHook("beforeEntities", (context, camera) => {
    ...
    const stairsLocation = new StairsLocation(4e3, 0);
  ...
  attachPlayer(player, namedLocation) {
    const entityManager = this.getEntityManager();
    let topLeft;
    if (namedLocation) {
    ...
    this.unsubscribeFromPlayerMoveFn = this.getEntityManager().emitter.subscribe("player_move", (player2) => {
      if (player2.getGameObject().getBox().getTopLeft().y < -3e3) {
...
const GIANT_TREE_MINI_TREE_POINTS = [
  {
    "x": 1165,
    "y": 1003.5
  },
...
const GIANT_TREE_MINI_TREE_BOXES = [
  [
    {
      "x": 1227,
      "y": 1053.5
...
class MiniTree extends DrawableEntity {
  constructor(x, y, scale) {
    ...
    for (const box2 of GIANT_TREE_MINI_TREE_BOXES) {
      const point1 = box2[0];
      const point2 = box2[1];
      const scaledPoint1 = {
...
class GiantTree extends DrawableEntity {
  constructor(x, y, scale = 2) {
    super();
    __publicField(this, "tree", null);
    __publicField(this, "miniTree", null);
    __publicField(this, "leftBottomExit");
    this.x = x;
    this.y = y;
    this.scale = scale;
    const imageBox = new Box(
...
class GiantTreeMap extends GameMap {
  constructor(game2) {
    super(game2.getCanvas());
    __publicField(this, "giantTree");
    __publicField(this, "unsubscribeFromPlayerMoveFn", null);
    this.game = game2;
  }
  initialize() {
    super.initialize();
    const entityManager = this.getEntityManager();
    const giantTree2 = new GiantTree(0, 0, 2.6);
    this.giantTree = giantTree2;
    entityManager.addToScene(giantTree2);
    entityManager.addEntity(giantTree2);
    const fire1 = new FireEntity(300, giantTree2.getHeight__TODO_DELETE_IT() - 480, 150, 150);
    entityManager.addToScene(fire1);
    entityManager.addEntity(fire1);
    const fire2 = new FireEntity(800, giantTree2.getHeight__TODO_DELETE_IT() - 230, 150, 150);
    entityManager.addToScene(fire2);
    entityManager.addEntity(fire2);
  }
  attachPlayer(player) {
    const entityManager = this.getEntityManager();
    const giantTree2 = this.giantTree;
    player.getGameObject().getBox().setTopLeft(new Position(400, giantTree2.getHeight__TODO_DELETE_IT() - 1800, 0));
    entityManager.addEntity(player, "player");
    entityManager.addToScene(player);
    const camera = new FollowPlayerCamera(window.innerWidth, window.innerHeight, player, {
      minHeight: 0,
      minWidth: 0,
      maxWidth: giantTree2.getWidth___TODO_DELETE_IT(),
      maxHeight: giantTree2.getHeight__TODO_DELETE_IT()
    });
    entityManager.addEntity(camera, "camera");
    this.getScene().camera = camera;
    this.unsubscribeFromPlayerMoveFn = entityManager.emitter.subscribe("player_move", (player2) => {
      if (player2.getGameObject().getBox().isCollide(this.giantTree.getLeftBottomExit())) {
...
class Game {
  constructor(canvas2) {
    ...
    __publicField(this, "testMap");
    __publicField(this, "homeMap");
    __publicField(this, "giantTreeMap");
    __publicField(this, "currentGameMap", null);
